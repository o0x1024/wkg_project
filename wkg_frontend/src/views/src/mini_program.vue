...
export default defineComponent({
  ...
  setup() {
    ...
    let curPage = ref(1)
    let pageSize = ref(10)
    let columns = types.getMiniTableColumns()
    ...
    const handleTableChange = (pag: any, _filters: any, _sorter: any) => {
      ...
      miniService.GetMiniInfo(curPage.value, pageSize.value, "", searchKey.value).then((res: any) => {
    ...
    const OnAllHasRead = () => {
      ...
      miniService.ReadAllFlagMiniInfo().then((res: any) => {
        if (res.data.code == 400) {
    ...
    const OnHasRead = () => {
      ...
      miniService.ReadFlagMiniInfoById(delist).then((res: any) => {
        if (res.data.code == 400) {
          message.error(res.data.msg)
        } else if (res.data.code == 200) {
          message.success(res.data.msg)
    ...
    const OnDelete = (id: number) => {
      miniService.DelMiniInfo(id).then((res: any) => {
        if (res.data.code == 400) {
          message.error(res.data.msg)
        } else if (res.data.code == 200) {
          message.success(res.data.msg)
    ...
    const InitData = () => {
      miniService.GetMiniInfo(curPage.value, pageSize.value, searchKey.value, searchKey.value).then((res: any) => {
        if (res.data.code == 400) {
          message.error(res.data.msg)
        } else if (res.data.code == 200) {
          rolesList.list = (res.data.data)
