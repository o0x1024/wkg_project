...
var local_wdsi model.WebDirScanType
...
func handResult() {
	...
	for _, vaule := range OpenResult {
		log.Println("Status[\x1b[32m200\x1b[0m]" + " URL[\x1b[36m" + vaule.URL + "\x1b[0m]" + " [content-length][" + strconv.Itoa(int(vaule.ContentLength)) + "]")

	}
	fmt.Printf("\x1b[1;32m%s\x1b[0m\n", "\n|----------------------------------------RESULT-------------------------------------|\n")
}

func WebDirScan(wdsi model.WebDirScanType, rCtx context.Context) {
	local_wdsi = wdsi
	...
	if wdsi.TargetDirPath != "" {
		...
		if err != nil {
			golog.Error("[webDirScan.go] line:130 read target from fileOp error! line: 94 err:", err)
		}
		...
		for i := 0; i < wdsi.ThreadNum; i++ {
			...
			go func(tId int, wdsi model.WebDirScanType, ctx context.Context) {
				//handler url
				var ok bool
				var tg chTarget
				tg, ok = <-chtar
		...
		for i := 0; i < wdsi.ThreadNum; i++ {
			wg.Add(1)
			go func(tId int, target string, wdsi model.WebDirScanType, ctx context.Context) {
				for len(payload) > 0 {
					select {
					case Uri := <-payload:
						_ = bar.Add(1)
					...
					case <-ctx.Done():
						//Glog.Println("recv parent context cancel signal web dir scan exit.")
						wg.Done()
						return
					default:
					}
...
func getHttpConnect(wdsi model.WebDirScanType) *http.Client {
...
func wdScan(tmpurl string, target string, wdsi model.WebDirScanType) {
	...
	if resp.StatusCode == 200 {
		...
		if !countFlag {
			bar.Clear()
			log.Println("\x1b[36m" + tmpurl + " --> " + strconv.Itoa(resp.StatusCode) + "    " + "size[" + strconv.Itoa(int(resp.ContentLength)) + "]" + "\x1b[0m")
		...
		if !countFlag {
			bar.Clear()
			log.Println("\x1b[36m" + tmpurl + " --> " + strconv.Itoa(resp.StatusCode) + "    " + resp.Header.Get("Location") + "\x1b[0m")
		...
		if !countFlag {
			bar.Clear()
			log.Println("\x1b[36m" + tmpurl + " --> " + strconv.Itoa(resp.StatusCode) + "\x1b[0m")
			otherResult = append(otherResult, tmpResult)
		}
	}
